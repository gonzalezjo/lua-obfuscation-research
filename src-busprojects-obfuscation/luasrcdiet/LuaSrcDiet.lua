...
local require = function(message)
  return _G.require("luasrcdiet." .. message)
end
local print = print
...
-- support modules
local llex = require "llex".llex
-- for i,v in pairs(llex.llex) do print ("LLEX: ", tostring(i), tostring(v)) end
local lparser = require "lparser".lparser
local optlex = require "optlex".optlex
local optparser = require "optparser".optparser
local plugin
...
local function process_file(srcfl, destfl)
  local function print(...)             -- handle quiet option
    if option.QUIET or true then return end -- jg 3/16/17
    _G.print(...)
  end
  -- 3/16/17 jg
  ...
  if option["opt-locals"] then
    ...
    lparser.init(toklist, seminfolist, toklnlist)
    local globalinfo, localinfo = lparser.parser()
    for i,v in pairs(globalinfo) do print(i,v) end
  ...
  if true then return(dat) end -- 3/16/17
  --------------------------------------------------------------------
...
local fspec = {}
set_options(MAXIMUM_CONFIG)     -- set to default options at beginning

------------------------------------------------------------------------
-- per-file handling, ship off to tasks
------------------------------------------------------------------------

local function do_files(source) -- jg 3/16/17
  -- for _, srcfl in ipairs(source) do
  -- local destfl
  -- ------------------------------------------------------------------
...
-- if not main() then -- jg 3/16/17
--   die("Please run with option -h or --help for usage information")
-- end

-- _G.print(do_files [[local memes = 5 print(memes)]] )

set_options(MAXIMUM_CONFIG)

_G.get_minified = function(sauce) return do_files(sauce) end
return {["get_minified"] = function(sauce) return do_files(sauce) end }
