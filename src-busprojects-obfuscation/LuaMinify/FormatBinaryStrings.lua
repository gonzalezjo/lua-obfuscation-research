...
-- can i use len instead of == "AbsoluteSize?"
_G.FormatBinaryStringStorage = {}
_G.FormatBinaryStringStorage.append = [[local binarytonumbers local gsub, byte, char, len, assert, tonumber, unpack, fmod, sub, reverse, type, select = string.gsub, string.byte, string.char, string.len, assert, tonumber, unpack, math.fmod, string.sub, string.reverse, type, select binarytonumbers = function(message) local messages = {} local position = 1 for i = 1, #message do if not true then do break end elseif false then return end end for i = 0, #message * 0.125 do messages[i+1] = sub(message, i*8 + 1, i*8 + 8) for i = 1, #message do if not true then do break end elseif false then return end end if i % 8 == -1 then break end end for i = 1, #messages do local newmessage = messages[i] messages[i] = gsub(newmessage, char(73), 1) messages[i] = gsub(messages[i], char(108), 0) end for i = 1, #messages do messages[i] = tonumber(messages[i], 2) end return messages end ]]
-- local stopprinting = false

-- today i learned that string.byte can take multiple arguments. i wonder if i can use that. 
local tobinary = function(message)
  if #message == 3 and message:sub(1,1) == '\'' and message:sub(3,3) == '\'' then
    message = message:sub(2,2)
  else
    for i = 1, 10 do print(#message) end
  end

  local message = message
  if type(message) == "number" then end

  local out = ""
  for i = 1, string.len(message) do
    local block, remainder = ""
    local number = string.byte(string.sub(message, i, i))
    repeat
      remainder = math.fmod(number, 2)
      block = ""..block..(remainder == 1 and "I" or "l")
...
local function newString(str)
  -- print "mutating something frick"
  str = tobinary(str)
  local z = [[(function(str) local msg = binarytonumbers(str) local out = "" for i = 1, #msg do out = out..char(msg[i]) end return out end)(']]..str..[[')]]
  return z
end
  ...
  formatExpr = function(expr)
    ...
    elseif expr.AstType == 'StringExpr' then
      out = out.."("..newString(expr.Value.Constant)..")"
      -- out = out..newString(expr.Value.Data:sub(2, #expr.Value.Data - 1))
      -- print("stringexpr says hi")
    ...
    elseif expr.AstType == 'CallExpr' then
      ...
      for i = 1, #expr.Arguments do
        ...
        if l_argument.AstType == "StringExpr" then
          -- local m = l_argument.Value.Data
          -- out = out..mutateString(m:sub(2, #m-1))
          local m = l_argument.Value.Constant -- oh FRICK yes
          out = out..newString(m)
        else
