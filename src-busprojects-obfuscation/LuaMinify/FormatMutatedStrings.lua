...
local function mutateString(str)
 	if #str <= 1 then return "([["..str.."]])" end
 	-- test \101 to make sure that its handled gracefully
 	-- and make sure im testing it from a file i obfuscate on the disk
	-- if str:match"\\" then return str end -- handling "\\" could be annoying (then again, i could just check if it's within 4 chars. that'd be hot wait im not thinking)

	local escapecharacters = 0
	for _ in str:gmatch("]") do
		escapecharacters = escapecharacters + 1
	end

	local open = "["..("="):rep(escapecharacters).."["
	local close = "]"..("="):rep(escapecharacters).."]"
	...
	local position = 1
	for i = 1, #str do -- the ridiculous 3 liner gsub was annoying to debug
	...
	formatExpr = function(expr)
		...
		elseif expr.AstType == 'StringExpr' then
			-- out = out..mutateString(expr.Value.Data:sub(2, #expr.Value.Data-1))
			local m = expr.Value.Constant
			out = out..mutateString(m)
			-- print(out)
			-- out = out..mutateString(expr.Value.Data)
			-- out = out..mutateString(expr.Value.Data:sub(2, #expr.Value.Data - 1))
		...
		elseif expr.AstType == 'StringCallExpr' then
			out = out..formatExpr(expr.Base) .. " "
			-- local m = expr.Arguments[1].Data
			out = out..mutateString(expr.Arguments[1].Constant)
			-- out = out..mutateString(m:sub(2, #m-1))
