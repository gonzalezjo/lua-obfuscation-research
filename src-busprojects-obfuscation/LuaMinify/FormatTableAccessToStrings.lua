-- test: table[1]:print()
-- test: table["1"]:print()
-- i should obfuscate numberIndexed[1] == 5
...
-- local dumptable = require'Helper.helper'.recursivelyPrintTable
...
formatExpr = function(expr)
	...
	elseif expr.AstType == 'DotsExpr' then
		-- print "Hi from dotsexpr"
		out = out.."..."

	elseif expr.AstType == 'CallExpr' then
		-- local skip = out:sub(#out) == "-"
		-- print ("Skip " .. tostring(skip))
		-- if skip == false then
		-- 	print "Reason: "
		-- 	print(out)
		-- end
		local isSelfCall = expr.Base and expr.Base.Indexer and expr.Base.Indexer == ":"
		...
		if isSelfCall then
			...
			do
				...
				while (not base.Name) do
					base = base.Base
				end
				name = base.Name
			end
			out = out .. formatExpr(expr.Base) .. "(" .. name .. ((#expr.Arguments > 0) and "," or "")
	...
	elseif expr.AstType == 'TableCallExpr' then
		if expr.Base.Indexer == ":" then
			...
			out = out..formatExpr(expr.Base).. "(" .. name .. ","
	...
	elseif expr.AstType == 'StringCallExpr' then
		if expr.Base.Indexer == ':' then
			...
			-- out = out .. "..expr.Ident.Data.."\"]")
		else
			out = out..formatExpr(expr.Base) .. " "
			out = out..expr.Arguments[1].Data
		end

	elseif expr.AstType == 'IndexExpr' then -- TODO: tonumber"index". handle 9e9 and stuff. ACTUALLY LUA DOES IT FOR ME WOO
		out = out..formatExpr(expr.Base).."["..formatExpr(expr.Index).."]"

	elseif expr.AstType == 'MemberExpr' then
		-- Indexer == "."
		if expr.Indexer == "." then
			out = out..formatExpr(expr.Base).."[\""..expr.Ident.Data.."\"]"
		elseif expr.Indexer == ":" then
			-- error(expr.Base.AstType)
			out = out..formatExpr(expr.Base)..expr.Indexer..expr.Ident.Data
		else
			error "Unsupported operation in FormatTableAccessToStrings."
		end
			-- print"\n\n"
			-- -- expr.Ident:Print("")
			-- print(expr.Ident.AstType)
			-- out = out..formatExpr(expr.Base).."[\""..expr.Ident.Data.."\"]"
			-- -- out = out.."(" .. formatExpr(expr.Base) .. ")"
			-- for i,v in pairs(expr.Ident) do print(i,v)  end
			-- print"\n\n"
			-- print("OUT: " .. out)
			-- error "self handling NOT SUPPORTED"
		-- out = out..formatExpr(expr.Base)..expr.Indexer..expr.Ident.Data
