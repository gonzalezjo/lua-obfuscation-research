...
function ParseLua(src)
	...
	--
  local function generateBinaryName()
    local length = 12
    local output = ""
    repeat output = output..(math.random()<0.5 and "l" or "I") until #output >= 12
    return output
  end
	...
	local function CreateScope(parent)
		...
		function scope:ObfuscateVariables(message)
      local binary = message and message:lower():match"binary"
			if not _G.renameVariables then return end
			for _, var in pairs(scope.LocalList) do
				local id = ""
				repeat
					id = generateBinaryName()
