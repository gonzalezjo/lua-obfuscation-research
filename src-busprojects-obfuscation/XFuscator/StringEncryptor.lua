return function(ast)
    ...
    local _, node = ParseLua(([[
        local floor, char, byte, gsub, concat, tonumber, unpack = math.floor, string.char, string.byte, string.gsub, table.concat, tonumber, unpack local decode = function(...)
            print("Numbers: " .. #{...})
            ...
            for i=1, length do
                ...
                -- print"D1"
                -- print("INT!: " .. int)
                inputstring[i]= char(floor(password[i] - byte(inputstring[i])))
            end
    ...
    for k, v in pairs(constantPoolAstNode.EntryList) do -- should do some trig shit for number encryption
        if v.Value then
            if v.Value.AstType == 'StringExpr' then
                ...
                local newNode = "local _ = concat { "
                newNode = newNode .. ("decode("..("%s,'%s')"):format(table.concat(passes, ","), table.concat(newstr, "','")))
                newNode = newNode:gsub("%(%,'", "('")
