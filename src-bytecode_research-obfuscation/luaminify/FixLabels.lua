...
local function Format_Beautify(ast)
	...
	formatExpr = function(expr)
		...
		elseif expr.AstType == 'BinopExpr' then
			local lhs = formatExpr(expr.Lhs)
			local rhs = formatExpr(expr.Rhs)
			if expr.Lhs.Replacement then
				lhs = expr.Lhs.Replacement:format(lhs)
			end
			if expr.Rhs.Replacement then
				Rhs = expr.Rhs.Replacement:format(rhs)
			end
			out = joinStatementsSafe(out, lhs) .. " "
			out = joinStatementsSafe(out, expr.Op) .. " "
			out = joinStatementsSafe(out, rhs)
	...
	formatStatement = function(statement)
		...
		elseif statement.AstType == 'LocalStatement' then
			...
			elseif statement.AstType == 'StaticStatement' then
				out = getIndentation() .. out.."static."
				for i = 1, #statement.LocalList do
					out = out..statement.LocalList[i].Name
					if i ~= #statement.LocalList then
						out = out..", static."
		...
		elseif statement.AstType == 'LabelStatement' then
			-- print "LABEL!"
			-- out = getIndentation() .. "label'" .. statement.Label .. '\'' .. EOL
			out = getIndentation() .. "label " .. statement.Label .. EOL
